...
class PayslipPreview extends Component {
    ...
    render() {
        if(this.state.requestInfo.length > 0) {

        var MonthText= "";
        let MonthName = ["January","February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
        for (let i = 0; i < MonthName.length; i++) {
            if ((this.state.requestInfo[0].PayslipForMonth - 1) === i) {
                MonthText = MonthName[i]
            }
        }
        var Notes = [];
        if (this.state.request.Notes !== undefined) {
            for (let i = 0; i < this.state.request.Notes.length; i++) {
                Notes.push({
                    notes: this.state.request.Notes[i]
                });
            }
        var date = new Date();
        var year = date.getFullYear();
            ...
            return (
                ...
                <div className="PaylistReport">
                    <table className="header">
                        <tbody><tr>
                            <td>Kính gửi Anh/ Chị-<i>To Mr./ Mrs. </i><strong>{this.state.request.EmployeeName}</strong></td>
                    ...
                    <table>
                        <tbody><tr>
                            ...
                            <tr>
                                <td className="text-center">Tháng <strong>{this.state.requestInfo[0].PayslipForMonth}</strong> - Năm {year}/ <i>Month <strong>{MonthText}</strong> - Year {year}</i></td>
                            </tr>
                        </tbody></table>
            ...
            return(
                <h1 className="text-center" style={{color: "grey", marginTop: "300px"}}>Đang tải, vui lòng chờ...</h1>
            );}
        } else {
            return(
                <h1 className="text-center" style={{color: "grey", marginTop: "300px"}}>Đang tải, vui lòng chờ...</h1>
